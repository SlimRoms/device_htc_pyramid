...
static struct convert_params toConvert[] = {
  { "contrast-max", "max-contrast" },
  { "contrast-min", "min-contrast" },
  { "contrast-def", "def-contrast" },
  { "brightness-max", "max-brightness" },
  { "brightness-min", "min-brightness" },
  { "brightness-def", "def-brightness" },
  { "saturation-max", "max-saturation" },
  { "saturation-min", "min-saturation" },
  { "saturation-def", "def-saturation" },
  { "sharpness-max", "max-sharpness" },
  { "sharpness-min", "min-sharpness" },
  { "sharpness-def", "def-sharpness" },
  { 0, 0 }
};
        ...
        const char *params)
{
    ...
    if (ret != 0)
      {
	ALOGE("%s: failed %d, say 0 anyway (FIXME!)\n", __func__, ret);
	ret = 0;
      }
    return ret;
}
